...
const reflectUtils_1 = require("./reflectUtils");
class Util extends appolo_inject_1.Util {
    ...
    static findReflectData(symbol, exported) {
        return reflectUtils_1.ReflectUtils.findReflectData(symbol, exported);
    }
    static findAllReflectData(symbol, exported) {
        return reflectUtils_1.ReflectUtils.findAllReflectData(symbol, exported);
    }
    static setReflectMetadata(key, value, target, propertyKey) {
        return reflectUtils_1.ReflectUtils.setReflectMetadata(key, value, target, propertyKey);
    }
    static getReflectMetadata(symbol, klass, propertyName, defaultValue) {
        return reflectUtils_1.ReflectUtils.getReflectMetadata(symbol, klass, propertyName, defaultValue);
    }
    static decorateReflectMetadata(key, value) {
        return reflectUtils_1.ReflectUtils.decorateReflectMetadata(key, value);
    }
}
exports.Util = Util;
