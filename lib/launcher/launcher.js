...
class Launcher {
    ...
    async initDynamicModules() {
        ...
        this._app.eventBeforeModulesLoad.fireEvent();
        await this._moduleManager.loadDynamicModules();
        this._app.eventModulesLoaded.fireEvent();
    }
    ...
    async initInjector() {
        ...
        await inject_2.Util.runRegroupByParallel(this._app.children, app => (Reflect.getMetadata(moduleDecorators_1.AppModuleOptionsSymbol, app) || {}).parallel, app => app.launcher.initInjector());
        this._app.eventBeforeInjectorInit.fireEvent();
        await this._injector.initialize({
            immediate: this._moduleOptions.immediate,
            parallel: this._moduleOptions.parallel
        });
        this._app.eventInjectorInit.fireEvent();
    }
    ...
    async initBootStrap() {
        ...
        await this._app.eventBeforeBootstrap.fireEventAsync();
        await inject_2.Util.runRegroupByParallel(this._app.children, app => (Reflect.getMetadata(moduleDecorators_1.AppModuleOptionsSymbol, app) || {}).parallel, app => app.launcher.initBootStrap());
        let bootstrapDef = this._injector.getDefinition(this._options.bootStrapClassId);
        if (!bootstrapDef) {
            await this._app.eventBootstrap.fireEventAsync();
    ...
    async _initFiles() {
        ...
        await utils_1.Promises.map(this._app.children, app => app.launcher._initFiles());
    ...
    _handleFn(fn, filePath) {
        ...
        let define = null;
        if (hasDefine) {
            this._app.eventsBeforeInjectRegister.fireEvent({ type: fn, filePath });
        ...
        this._app.eventsEventClassExport.fireEvent({ type: fn, filePath });
    ...
    _handlePipeLines() {
        for (let i = 0, len = (this._app.discovery.exported || []).length; i < len; i++) {
            ...
            if (item.define) {
                ...
                this._app.eventsInjectRegister.fireEvent({
                    type: item.fn,
                    filePath: item.path,
