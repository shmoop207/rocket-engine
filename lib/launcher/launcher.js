...
class Launcher {
    ...
    async initDynamicModules() {
        ...
        await this._app.events.beforeModulesLoad.fireEvent();
        await this._moduleManager.loadDynamicModules();
        await this._app.events.modulesLoaded.fireEvent();
    }
    ...
    async initBootStrap() {
        ...
        if (!bootstrapDef) {
            await this._app.events.bootstrap.fireEventAsync();
    ...
    _handleFn(fn, filePath) {
        ...
        this._app.events.classExport.fireEvent({ type: fn, filePath });
    ...
    _handlePipeLines() {
        for (let i = 0, len = (this._app.discovery.exported || []).length; i < len; i++) {
            ...
            if (item.define) {
                ...
                this._app.events.injectRegister.fireEvent({
                    type: item.fn,
                    filePath: item.path,
