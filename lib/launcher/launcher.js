...
const bootstrap_1 = require("../decoretors/bootstrap");
...
class Launcher {
    ...
    createModuleManager() {
        this._moduleManager = new modules_1.ModuleManager(this._options, this._injector, this._plugins);
    ...
    _handleFn(fn, filePath) {
        ...
        if (define) {
            this._app.fireEvent(events_1.Events.BeforeInjectRegister, fn, filePath, define);
            this._injector.register(fn, null, filePath);
            this._files.push(filePath);
            this._app.fireEvent(events_1.Events.InjectRegister, fn, filePath, define);
        }
        if (Reflect.hasMetadata(bootstrap_1.BootstrapSymbol, fn)) {
