...
class Module {
    ...
    _fireClassExportEvents() {
        if (!this._app.hasListener(events_1.Events.ClassExport) && !this._app.hasListener(events_1.Events.InjectRegister)) {
            return;
        }
        _.forEach(this._app.parent.exportedClasses, item => {
            if (Reflect.hasMetadata(index_2.InjectDefineSymbol, item.fn)) {
                this._app.fireEvent(events_1.Events.InjectRegister, item.fn, item.path);
            }
            this._app.fireEvent(events_1.Events.ClassExport, item.fn, item.path);
        });
    }
    _setDefinitions() {
