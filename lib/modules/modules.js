...
class ModuleManager {
    ...
    async loadDynamicModules() {
        ...
        await util_1.Util.runRegroupByParallel(this._modules, loader => loader.moduleOptions.parallel, module => this._loadModule(module));
    }
    async initAfterInjectDynamicModules() {
        await util_1.Util.runRegroupByParallel(this._modules, loader => loader.moduleOptions.parallel, module => module.afterLaunch());
    }
    ...
    async load(modules) {
        let moduleParams = modules.map((item) => {
            if (utils_1.Classes.isClass(item)) {
                ...
                return Object.assign({ options: {}, moduleOptions: {} }, item);
            }
        });
        let [dynamicModules, staticModules] = utils_1.Arrays.partition(moduleParams, module => !!module.module);
    ...
    _loadStaticModule(moduleParams) {
        ...
        let args = util_1.Util.getFunctionArgs(moduleParams.fn), lastArg = args[args.length - 1], isCallback = false;
