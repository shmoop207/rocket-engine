...
class ModuleLoader {
    constructor(_moduleParams, _parenInjector) {
        ...
        this._moduleOptions = utils_1.Objects.defaults({}, _moduleParams, {
    ...
    _handleExports(app) {
        ...
        moduleExports.forEach(item => {
            ...
            this._module.app.eventModuleExport.fireEvent(eventDto);
            this._module.app.parent.eventModuleExport.fireEvent(eventDto);
        });
    }
    _handleFileExport() {
        this._module.fileExports.forEach(fn => {
            this._module.app.parent.discovery.add({ path: "", fn, define: inject_1.Util.getClassDefinition(fn) });
        });
    }
    _handleImports(app) {
